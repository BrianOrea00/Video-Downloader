...
class VideosTab:
    ...
    def _build_row_structure(self, row):
        ...
        row.size_chip.pack(side="left", padx=(0, 6))
        ...
        row.date_chip.pack(side="left")
        row.date_chip_label = ctk.CTkLabel(
            row.date_chip,
            text="",
            font=ctk.CTkFont(size=11),
            text_color=theme_manager.get_color("greige")
        )
        row.date_chip_label.pack(padx=6, pady=2)
    ...
    def _update_row(self, row, file_info, index):
        ...
        row.name_label.configure(text=filename)

        row.size_chip_label.configure(text=file_info['size'])
        row.date_chip_label.configure(text=file_info['modified'])

        row.thumb_icon.configure(image=icon_manager.get("video_sm"))

        row.play_btn.configure(command=lambda p=file_info['path'], n=file_info['name']: self.play_file(p, n))
        row.delete_btn.configure(command=lambda p=file_info['path'], n=file_info['name']: self.delete_file(p, n))
