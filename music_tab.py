...
class MusicTab:
    ...
    def show_empty_state(self):
        try:
            for row in self.row_pool:
                row.pack_forget()
            if self.empty_state and not self.empty_state.winfo_ismapped():
    ...
    def filter_files(self):
        ...
        filtered = [f for f in self.current_files
                    if search_term in f['title'].lower() or search_term in f['artist'].lower()]

        # Ensure pool has enough rows
        while len(self.row_pool) < len(filtered):
            row = self._create_row_widget()
            self.row_pool.append(row)

        # Update each row
        for i, file_info in enumerate(filtered):
            row = self.row_pool[i]
            self._update_row(row, file_info, i)
            if not row.winfo_ismapped():
                row.pack(fill="x", pady=4)

        # Hide extras
        for i in range(len(filtered), len(self.row_pool)):
    ...
    def _create_row_widget(self):
        ...
        row.artist_label = None
        row.duration_chip_label = None
        row.size_chip_label = None
        row.play_btn = None
        row.delete_btn = None
    ...
    def _build_row_structure(self, row):
        ...
        row.thumb_icon = ctk.CTkLabel(icon_frame, text="")
        row.thumb_icon.pack()
        ...
        row.artist_label.pack(anchor="w")
        ...
        # Duration chip
        row.duration_chip = ctk.CTkFrame(
            chips_frame,
            fg_color=theme_manager.get_color("surface"),
            border_width=1,
        ...
        row.duration_chip.pack(side="left", padx=(0, 6))
        row.duration_chip_label = ctk.CTkLabel(
            row.duration_chip,
            text="",
            font=ctk.CTkFont(size=10),
            text_color=theme_manager.get_color("accent")
        )
        row.duration_chip_label.pack(padx=6, pady=2)

        # Size chip
        row.size_chip = ctk.CTkFrame(
            chips_frame,
            fg_color=theme_manager.get_color("surface"),
            border_width=1,
            border_color=theme_manager.get_color("border"),
        ...
        row.size_chip.pack(side="left")
        row.size_chip_label = ctk.CTkLabel(
            row.size_chip,
            text="",
            font=ctk.CTkFont(size=11),
            text_color=theme_manager.get_color("greige")
        )
        row.size_chip_label.pack(padx=6, pady=2)

        # Actions
        actions = ctk.CTkFrame(row, fg_color="transparent")
        actions.pack(side="right", padx=12, pady=12)

        row.play_btn = ctk.CTkButton(
            actions,
            text="",
    ...
    def _update_row(self, row, file_info, index):
        title = file_info['title']
        if len(title) > 45:
            title = title[:42] + "..."
        row.title_label.configure(text=title)

        row.artist_label.configure(text=file_info['artist'])
        row.duration_chip_label.configure(text=file_info['duration'])
        row.size_chip_label.configure(text=file_info['size'])

        row.thumb_icon.configure(image=icon_manager.get("music_sm"))

        row.play_btn.configure(command=lambda p=file_info['path'], t=file_info['title']: self.play_file(p, t))
        row.delete_btn.configure(command=lambda p=file_info['path'], t=file_info['title']: self.delete_file(p, t))
